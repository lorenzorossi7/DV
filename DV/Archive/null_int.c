...
// for the maple routines giving the e.o.m.
// 
//=============================================================================
...
//=============================================================================
int eval_f_1d(time_str *ts, double x, double *dx, double *f, double *df)
{
   level *l;
   grid *g,*cg=0;
   int i,nx;
...
void sf_null_int(char *args)
{
   ...
   while (steps--)
   {
      ...
      while(t<fabs(t0))
      {
         if (!eval_f_1d(ts[ALPHA_Z0],0,0,&fn[ALPHA_Z0],0)) 
      ...
      while(ts[0]->next)
      {
         ...
         for (i=0;i<NUM_VARS;i++) 
         {
            ...
            while (ts[i] && !eval_f_1d(ts[i],x,&dx,&fn[i],&fn_x[i])) ts[i]=ts[i]->next;
            if (!ts[i]) goto done;
            ...
            {
               qts=ts[i]->prev;
               while(qts && !eval_f_1d(qts,x,0,&fnm1[i],0)) qts=qts->prev;
               if (!qts) can_dis=0; else { y=qts->time; dtm1=ts[i]->time-y; qts=qts->prev; }
               while(qts && !eval_f_1d(qts,x,0,&fnm2[i],0)) qts=qts->prev;
               if (!qts) can_dis=0; else dtm2=y-qts->time;
            }
   
            prev_t=ts[i]->time;
            ts[i]=ts[i]->next;
            while (ts[i] && !eval_f_1d(ts[i],x,0,&fnp1[i],&fnp1_x[i])) ts[i]=ts[i]->next;
            if (!ts[i]) goto done;
   
            if (eps_dis>0 && can_dis)
            {
               qts=ts[i]->next;
               while(qts && !eval_f_1d(qts,x,0,&fnp2[i],0)) qts=qts->next;
               if (!qts) can_dis=0; else { y=qts->time; dtp1=y-ts[i]->time; qts=qts->next; }
               while(qts && !eval_f_1d(qts,x,0,&fnp3[i],0)) qts=qts->next;
